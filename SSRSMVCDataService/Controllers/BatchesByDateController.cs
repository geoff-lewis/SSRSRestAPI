...
namespace SSRSMVCDataService.Controllers
{
    ...
    [ApiController]
    public class BatchesByDateController : ControllerBase
    {
        ...
        public BatchesByDateController(IWebHostEnvironment webHostEnvironment)
        {
            m_WebHostEnvironment = webHostEnvironment;
        }

        public ActionResult Image(DateTime date, string numberOfBatches = "4")
        {
            ...
            var batchesImage = GetBatchesBitmap($"{date.ToLongDateString()} {date.ToShortTimeString()}", Convert.ToInt32(numberOfBatches));
            return File(ImageToByteArray(batchesImage), "image/png");
        }

        private Bitmap GetBatchesBitmap(string date, int numberOfBatches)
        {
            var totalLength = 100;
            var ran = new Random();
            ...
            using(var g = Graphics.FromImage(image))
            {
                ...
                g.DrawString(date, new Font("Arial", 16), new SolidBrush(Color.Black), new PointF(20, 20));
